...
namespace SmartPTT_API
{
    class FireAlarmWatcher
    {
        ...
        private void fileWatchFireAlarm()
        {
            ...
            watchFireLog.NotifyFilter = NotifyFilters.Size | NotifyFilters.LastWrite;
            #region Original
            //watchFireLog.Changed += new FileSystemEventHandler(OnChanged);
            #endregion
            watchFireLog.Changed += new FileSystemEventHandler(OnLineChange);
        ...
        private void OnLineChange(object sender, FileSystemEventArgs e)
        {
            ...
            #endregion
            processTxtFile(txtFilePath);
            foreach (var item in Constants.Config.fireRadioList)
            {
        ...
        private void processTxtFile(string filePath)
        {
            ...
            finalList.Clear();
            string lastLine = File.ReadAllLines(filePath).Last();
            finalList.Add(lastLine);
